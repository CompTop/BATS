...
auto prepare_ChainComplex(
	...
	for (size_t k = 0; k < C.maxdim() + 1; ++k) {
		...
		// 		return valsk[i].second > valsk[j].second;
		// 	}
	 	// );
		perm[k] = bats::util::sortperm(
			F.vals(k).begin(),
...
auto prepare_ChainComplex(
	...
	for (size_t k = 0; k < C.maxdim() + 1; ++k) {
		...
		perm[k] = bats::util::sortperm(
			...
			valsk.size(),
			[&](const size_t i, const size_t j) {
				return valsk[i].second == valsk[j].second ? X.rlex_cmp(k, j, k, i) : valsk[i].second > valsk[j].second;
			}
	 	);
		// perm[k] = bats::util::sortperm(
		// 	F.vals(k).begin(),
	...
	std::sort(
		filt_order.begin(),
		filt_order.end(),
		[&X](const rfilt_val<T>& a, const rfilt_val<T>& b) {
			return (a.val != b.val) ?
				a.val < b.val : // increasing value order
				(a.entry && b.entry) ? // else, if both entries
				X.lex_cmp(a.dim, a.cind, b.dim, b.cind) : // use lexicographical comparison
				(a.entry ^ b.entry) ? // one entry and one removal
				a.entry : // process entry first - doesn't matter
				(a.dim == b.dim) ? // else, both are removals - check if dim is the same
				a.ind > b.ind : // process later index first
				// a.dim > b.dim; // otherwise, we process larger dimension first for removal
				X.rlex_cmp(a.dim, a.cind, b.dim, b.cind);
			}
	);
	// std::cout << "filt_order: " << std::endl;
	// for (auto& rval : filt_order) {
