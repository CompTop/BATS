...
// Kendall tau distance of permutations in a vector
template<typename T>
std::vector<size_t> Kendall_tau(const std::vector<std::vector<T>>& perms)
{
    std::vector<size_t> ken_dists;
    for (auto perm: perms) {
        ken_dists.emplace_back(Kendall_tau(perm));
    }
    return ken_dists;
}
